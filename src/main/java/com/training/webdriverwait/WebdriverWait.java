...
public class WebdriverWait extends DriverUtility{
	...
	public static void waitForElementList(List<WebElement> element)
	{
		try
		{
			WebDriverWait waitForElementList = new WebDriverWait(driver, timeout);
			waitForElementList.until(ExpectedConditions.visibilityOf((WebElement) element));
		}
		catch(Exception ex)
		{
			Assert.fail(timeout+" Seconds Waited For Elements '"+element+"'  "+ex.getMessage());
		}
	...
	public static void veryLongWait(){
		try {
			Thread.sleep(30000);
		} catch (InterruptedException e) {
			e.printStackTrace();
